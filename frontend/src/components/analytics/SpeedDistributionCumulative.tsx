...
interface SpeedDistributionCumulativeProps {
  directory: string;
}

export default function SpeedDistributionCumulative({ directory }: SpeedDistributionCumulativeProps) {
    ...
    <div>
      {limits.map((l) => (
        <div key={l.speed_limit} style={{ marginBottom: "1rem" }}>
          <img
            src={`/api/analytics/speed-distribution-cumulative-chart?directory=${encodeURIComponent(directory)}&speed_limit=${l.speed_limit}`}
            alt={`Cumulative speed distribution for ${l.speed_limit} km/h limit`}
